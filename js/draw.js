...
*/
var dom={t:$('#drawTitle'),c:$('#drawContentBox')};
cp2y.draw={
bt:10032,
clear:function(){this.setPage(1);},
setPage:function(p){localStorage.setItem('drawPage',p);},
getPage:function(){
        ...
        success:function(data){
            ...
            for(i;i<len;i++){
                ...
                for(j;j<jlen;j++){
                    ...
                    var b1,b2;
                    if(data2[j].bqcResult){
                      b1=data2[j].bqcResult.substr(0,1);
                      b2=data2[j].bqcResult.substr(-1);
                      html.push('<tr><td>半全场：</td><td>'+(b1==3?'胜':b1==1?'平':'负')+(b2==3?'胜':b2==1?'平':'负')+'</td><td>奖金'+data2[j].bqcSp+'</td></tr>');
                    }else{
                      html.push('<tr><td>半全场：</td><td>--</td><td>奖金'+data2[j].bqcSp+'</td></tr>');
                    }
                    html.push('</table>');
                    html.push('</li>');
                }
