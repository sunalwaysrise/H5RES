var bd={
  ...
  pd:function(){
    if(this.money==0){return cp2y.dialog.alert('请选择支持的结果');}
    $("#Warp").hide();
    $("#pdDetail").show();
    var h=[],i,n,m,t=[],d,t;
    h.push('<table class="jcScheme2 jcScheme3"><thead><tr><td>编号</td><td>主队VS客队</td><td>备注</td></tr></thead><tbody>');
    for(i in this.schemes){
      if(this.schemes[i].v.length==1){
        n="自选";
        m=this.n2m(this.schemes[i].v[0])+'('+this.schemes[i].r[0]+')';
        t=this.schemes[i];
      }else{
        d=this.schemes[i];
        n="配对";
        m=this.n2m(this.schemes[i].v[0])+'('+this.schemes[i].r[0]+') '+this.n2m(this.schemes[i].v[1])+'('+this.schemes[i].r[1]+')';
      }
      h.push('<tr><td rowspan="2">'+this.schemes[i].name+'</td><td>'+this.schemes[i].g+'VS'+this.schemes[i].h+'</td><td rowspan="2">'+n+'场次</td></tr><tr><td>'+m+'</td></tr>');
    }
    h.push('</tbody></table><table class="jcScheme2 jcScheme3 mt12"><thead><tr><td>组合详情</td><td>倍数</td><td>理论奖金</td></tr></thead><tbody>');
    
    h.push('<tr><td>'+d.name+this.n2m(d.v[0])+'('+d.r[0]+')<br/>'+t.name+this.n2m(t.v[0])+'('+t.r[0]+')'+'</td><td>'+this.mul+'</td><td>'+(2*d.r[0]*t.r[0]*this.mul).toFixed(2)+'</td></tr>');
    h.push('<tr><td>'+d.name+this.n2m(d.v[1])+'('+d.r[1]+')<br/>'+t.name+this.n2m(t.v[0])+'('+t.r[0]+')'+'</td><td>'+this.mul+'</td><td>'+(2*d.r[1]*t.r[0]*this.mul).toFixed(2)+'</td></tr>');
    
    h.push('</tbody></table>');
    $("#pdDetailCon").html(h.join(''));
  },
  closePd:function(){
    $("#Warp").show();
    $("#pdDetail").hide();
  ...
  addContent: function (e) {
    ...
    if(d.hostV==403){
      t1='主不败:让球胜平负(+1)下的[主胜],包含所有主队胜出或战平的赛事结果';
    }else if(d.hostV==3){
      t1='主胜:胜平负的[主胜],包含所有主队胜出的赛事结果';
    }
    if(d.guestV==0){
      t2='客胜:胜平负[客胜],包含所有客队胜出的赛事结果';
    }else if(d.guestV==400){
      t2='客不败:让球胜平负(-1)下的[客胜],包含所有客队胜出和平局的赛事结果';
    }
