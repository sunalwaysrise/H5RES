...
cp2y.quick={
  ...
  user:{
    ...
    updataPhoneBox:function(){
      var html=[];
      html.push('<header class="header"><a class="GoIndex" onclick="cp2y.quick.closeBox();">关闭</a><h2>绑定手机</h2></header>');
      html.push('<div class="userPartBox1"><div id="userTip31" ></div>');
      html.push('<input type="tel" id="Qmobile" placeholder="绑定手机可免费定制中奖通知、找回密码" class="input4 mt40" />');
      html.push('<a class="btn1 mt10" onclick="cp2y.quick.user.getVeri2()">确认</a>');
      html.push('</div>');
      userUtil.html(html.join('')).show();
      Warp.hide();
    },
    getVeri2:function(){
      var val=$("#Qmobile").val().trim();
      cp2y.dialog.clearTip();
      if(val.isPhone()){
        $.ajax({
          url:WebAppUrl.HOME_APP_URL+"/user/sendmsg",
          ...
          success:function(data){
            ...
            if(data.flag==1){
              var html=[];
              html.push('<header class="header"><a class="GoIndex" onclick="cp2y.quick.closeBox();">关闭</a><h2>输入验证码</h2></header>');
              html.push('<div class="userPartBox1">');
              html.push('<div id="userTip31"></div>');
              html.push('<div class="yourNumber mt40">验证码已经发送到<span>'+val+'</span></div>');
              html.push('<p id="resendVeri"><span>60</span> 秒后重新发送</p>');
              html.push('<input type="text" id="Qveri" placeholder="请输入手机验证码" class="input4" />');
              html.push('<a onclick="cp2y.quick.user.updataPhone()" class="btn1 mt10">提交验证码</a>');
              userUtil.html(html.join('')).show();
              Warp.hide();
              qCountDown.t=60;
              setTimeout('qCountDown.init()',1000);
    ...
    updataPhone:function(){
      var validateCode=$("#Qveri").val().trim();
      if(!validateCode){
        return cp2y.dialog.tip1('验证码不能为空');
      }else{
        $.ajax({
    ...
    updataIdBox:function(){
      var html=[];
      html.push('<header class="header"><a class="GoIndex" onclick="cp2y.quick.closeBox();">关闭</a><h2>个人信息</h2></header>');
      html.push('<div class="userPartBox1"><div id="userTip31" ></div>');
      html.push('<input type="text" id="QupdataNick" placeholder="真实姓名" class="input4 mt40" />');
      html.push('<input type="text" id="QupdataId" placeholder="身份证号" class="input4 nobt" />');
      html.push('<a class="btn1 mt10" onclick="cp2y.quick.user.updataId()">绑定</a>');
      html.push('</div>');
      userUtil.html(html.join('')).show();
      Warp.hide();
    },
    updataId:function(){
      var d={
        name:$("#QupdataNick").val().trim(),
        identityNumber:$("#QupdataId").val().trim()
      };
      if(!d.name || !d.identityNumber){return cp2y.dialog.tip1("信息不能为空");}
      if(d.identityNumber.isID()){return cp2y.dialog.tip1("身份证号码非法");}
      this._updataId(d);
    },
    _updataId:function(d){
      $.ajax({
