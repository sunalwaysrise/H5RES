...
cp2y.discover={
	...
	_base:function(data){
		...
		if(data.flag==1){
			...
			if(m){
			var html=[],mt=new Date(Number(String(m.mt)+"000")),len,MT=mt.getFullYear()+'-'+(mt.getMonth()+1).addZero()+'-'+(mt.getDate()).addZero()+' '+(mt.getHours()).addZero()+':'+(mt.getMinutes()).addZero()+':'+(mt.getSeconds()).addZero();
			this.mid=m.mid;
