...
_.a0={
	...
	bet:function(){
		...
		html0.push('<p>百位</p>');
		for(i;i<10;i++){
			html0.push('<a class="WW1 gb" onclick="cp2y.buy.select(this,3)" >'+i+'</a>');
		}
		i=0;
		html0.push('<p>十位</p>');
		for(i;i<10;i++){
			html0.push('<a class="WW2 gb" onclick="cp2y.buy.select(this,2)" >'+i+'</a>');
		}
		i=0;
		html0.push('<p>个位</p>');
		for(i;i<10;i++){
			html0.push('<a class="WW3 gb" onclick="cp2y.buy.select(this,1)" >'+i+'</a>');
		}
		return html0.join('');
	},
	...
	outGet:function(){
		var w1=cp2y.util.getArgs2('w1').split(','),
			w2=cp2y.util.getArgs2('w2').split(','),
			w3=cp2y.util.getArgs2('w3').split(','),
			i=0,o=$('.WW1');
		for(i;i<10;i++){
...
_.a5={
	hasOutGet:false,
	playName:_.playName,playType:"胆拖-直选",input:"directDraw",n:'组6',num:2,mul:4,bet:_.a4.bet,select:_.a3.select,count:function(){
		var d=[],t=[],i=0,o=this.getBall(),len=o.length,units=0,s=1;
		for(i;i<len;i++){
			if(o.eq(i).hasClass("rb")){
				d.push(o.eq(i).html());
