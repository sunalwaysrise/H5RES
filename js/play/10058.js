...
var _={
  ...
  playTypes: function(a){
    ...
    html.push('<a data="a0" href="' + WebAppUrl.HOME_APP_URL + '/lottery/10058?type=a0" ' + v0 + ' data2="让分胜负">让分胜负</a>');
    html.push('<a data="a1" href="' + WebAppUrl.HOME_APP_URL + '/lottery/10058?type=a1" ' + v1 + ' data2="胜负">胜负</a>');
    html.push('<a data="a2" href="' + WebAppUrl.HOME_APP_URL + '/lottery/10058?type=a2" ' + v2 + ' data2="大小分">大小分</a>');
    html.push('<a data="a3" href="' + WebAppUrl.HOME_APP_URL + '/lottery/10058?type=a3" ' + v3 + ' data2="胜分差">胜分差</a>');
    return html.join('');
  }
};
_.a0={
      ...
      success: function (data){
        ...
        for (i; i < len; i++) {
          ...
          for(j; j < jlen; j++){
            ...
            t=new Date(data2[j].endTime);
            html.push('<time>'+t.getHours().addZero()+':'+t.getMinutes().addZero()+'截止</time>');
      ...
      success: function (data){
        ...
        for (i; i < len; i++) {
          ...
          for(j; j < jlen; j++){
            ...
            t=new Date(data2[j].endTime);
            html.push('<time>'+t.getHours().addZero()+':'+t.getMinutes().addZero()+'截止</time>');
            html.push('</strong><div><p class="dxf"><em>'+data2[j].hostName+':'+data2[j].guestName+'</em><em>预设分:'+data2[j].basePoint+'</em></p>');
    ...
    $.ajax({
      ...
      success: function (data){
        ...
        for(i; i < len; i++){
          ...
          for (j; j < jlen; j++) {
            t=new Date(data2[j].endTime);
            t=t.getHours().addZero()+':'+t.getMinutes().addZero();
            html.push('<li class="' + data2[j].leagueName + '"><div class="jc_line1 jc_line12 jc_line5"  data="{no:' + data2[j].matchCode + ',end:false,name:\'' + data[i].dayOfWeekStr+data2[j].matchCode.substr(8,3)+ '\',h:\'' + data2[j].hostName + '\',g:\'' + data2[j].guestName + '\',rq:\'' + data2[j].rate + '\',dw:\'' + data[i].dayOfWeekStr + '\'}">');
            html.push('<strong><em>' + data2[j].leagueName + '</em>');
            html.push('<var>'+data2[j].matchCode.substr(8,3)+'</var>');
            html.push('<time>' + t + '截止</time>');
            html.push('</strong><div class="dbStyle1" >');
            html.push('<p onclick="cp2y.buy.bdShow(this);"><span>'+data2[j].hostName+'</span><span>VS</span><span>'+data2[j].guestName+'</span></p>');
            html.push('<code></code>');
